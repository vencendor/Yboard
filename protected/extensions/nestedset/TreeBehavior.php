    ...
    private function moveNode($sibling, $after = false, $aschild = false) {
        ...
        if ($movenode->getLeftValue() < $compare) {
            ...
            if ($after) {
                ...
                $move = $aschild ? $sibling->getRightValue() - $movenode->getRightValue() - 1 : $sibling->getRightValue() - $movenode->getRightValue();
            } else { // before
                $upper = $sibling->getLeftValue() - 1;
                $move = $aschild ? $sibling->getLeftValue() - $movenode->getRightValue() : $sibling->getLeftValue() - $movenode->getRightValue() - 1;
            }
            // sibling or child
            $upper += $aschild;
            // direction
            ...
            if ($after) {
                ...
                $move = $aschild ? $movenode->getLeftValue() - $sibling->getRightValue() : $movenode->getLeftValue() - $sibling->getRightValue() - 1;
            } else { // before
                $lower = $sibling->getLeftValue();
                $move = $aschild ? $movenode->getLeftValue() - $sibling->getLeftValue() + 1 : $movenode->getLeftValue() - $sibling->getLeftValue();
            }
            // sibling or child
            $lower += $aschild;
            // direction
